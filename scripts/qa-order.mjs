...
const run=async()=>{
 ...
 const order=await p.evaluate(()=>[...document.querySelectorAll('.deck-slide')].map(s=>s.id||(s.querySelector('h2')?.textContent?.slice(0,15))||s.className.toString().slice(0,25)))
