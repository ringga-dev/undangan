...
const run=async()=>{
 for(const [slug,name] of styles){
  ...
  await p.goto(`${BASE}/${slug}/?to=any&yanto`,{waitUntil:'networkidle',cache:'no-store'}).catch(()=>{})
  await p.waitForTimeout(1200)
  const r=await p.evaluate(()=>({g:document.querySelector('.hero .guest')?.textContent||document.querySelector('.guest')?.textContent||'',nav:!!document.querySelector('.navbar')}))
